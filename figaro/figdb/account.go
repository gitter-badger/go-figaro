...
func (db DB) SaveAccount(root []byte, account *figaro.Account) (newroot []byte, err error) {
	var buf []byte
	buf, err = account.Encode()
	if err != nil {
		return
	}
	newroot, err = db.State.Set(root, account.Address.Bytes(), buf)
	return
}

// FetchAccount returns an account from the database
func (db DB) FetchAccount(root []byte, address figaro.Address) (account *figaro.Account, err error) {
	var buf []byte
	buf, err = db.State.Get(root, address.Bytes())
	if err != nil {
		return
	}
	if len(buf) > 0 {
...
func (db DB) ProveAccount(root []byte, address figaro.Address) (account *figaro.Account, proof [][][]byte, err error) {
	var buf []byte
	buf, proof, err = db.State.GetAndProve(root, address.Bytes())
	if err != nil {
		return
	}
	if len(buf) > 0 {
...
func ValidateAccount(root []byte, account *figaro.Account, proof [][][]byte) bool {
	...
	return figdb.ValidateState(root, account.Address.Bytes(), buf, proof)
}
...
func (db DB) SaveAccountStorage(root []byte, account *figaro.Account, key, data []byte) (newroot []byte, err error) {
