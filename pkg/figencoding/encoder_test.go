...
import (
	...
	"github.com/figaro-tech/figaro/pkg/figencoding"
)
...
func (s *SelfMarshaler) MarshalDeterministicBinary(enc *figencoding.Encoder) ([]byte, error) {
	return enc.EncodeList(s.Name, s.Age), nil
}

func ExampleDeterministicBinaryMarshaler() {
	s := &SelfMarshaler{Name: "Bob", Age: 37}
	enc := &figencoding.Encoder{}
	b, _ := enc.Encode(s)
	fmt.Printf("% #x\n", b)
...
func BenchmarkEncoder_Encode_bytesSliceSlice(b *testing.B) {
	enc := &figencoding.Encoder{}
	t := [][][]byte{{{0xff, 0xfe}, {0xcd, 0x03}}, {{0xff, 0xfe}, {0xcd, 0x03}}}
	for i := 0; i < b.N; i++ {
		enc.Encode(t)
	}
}
...
// func TestEncoder_EncodeBytesSliceSlice(t *testing.T) {
// 	type args struct {
// 		ddd [][][]byte
// 	}
// 	tests := []struct {
...
// 		want []byte
// 	}{
// 		{"A few byte slice slices", args{[][][]byte{{{0xff, 0xfe}, {0xcd, 0x03}}, {{0xff, 0xfe}, {0xcd, 0x03}}}}, []byte{0xce, 0xc6, 0x82, 0xff, 0xfe, 0x82, 0xcd, 0x03, 0xc6, 0x82, 0xff, 0xfe, 0x82, 0xcd, 0x03}},
// 	}
// 	for _, tt := range tests {
// 		t.Run(tt.name, func(t *testing.T) {
// 			enc := &figencoding.Encoder{}
// 			if got := enc.EncodeBytesSliceSlice(tt.args.ddd); !reflect.DeepEqual(got, tt.want) {
// 				t.Errorf("Encoder.EncodeBytesSliceSlice() = % #x, want % #x", got, tt.want)
// 			}
// 		})
// 	}
// }

// func BenchmarkEncoder_EncodeBytesSliceSlice(b *testing.B) {
// 	enc := &figencoding.Encoder{}
// 	t := [][][]byte{{{0xff, 0xfe}, {0xcd, 0x03}}, {{0xff, 0xfe}, {0xcd, 0x03}}}
// 	for i := 0; i < b.N; i++ {
// 		enc.EncodeBytesSliceSlice(t)
// 	}
// }
